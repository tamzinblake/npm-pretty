...
function printBinaryOp (operator, first, second) {
...
function printDot (ast) {
  printBinaryOp('.' ,ast.expression ,ast.property)
}

function printSeq (ast) {
  printBinaryOp(',' ,ast.first ,ast.second)
}
...
function printExpression (ast ,oneLine) {
  ...
  switch (ast.__type) {
    ...
    default: print('\'Crazy node detected!: ' + ast.__type + '\'')
  }
}
