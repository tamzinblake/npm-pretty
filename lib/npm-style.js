...
function unaryNeedsSemi (op) {
  return op == '-' || op == '+'
}

function unaryNeedsSpace (op) {
  return op.match(/[a-z]/)
}
...
print.pushBuffer = function (opts) {
  ...
  print.commentsStack.push(print.comments)
  print.buffer = str
}
...
print.popBuffer = function () {
  ...
  print.comments = print.commentsStack.pop()
  return rv
}
...
print.make('list' ,function (arr ,oneLine) {
  for (var i = 0 ,l = arr.length ;i < l ;i++) {
    if (arr[i].start && arr[i].start.comments_before
                     && arr[i].start.comments_before.length) {
      print(' ')
      print.commentsBefore(arr[i].start.comments_before)
    }
    if (i) {
      if (oneLine) {
        print(' ,')
        ...
        if (!print.commentNewline) print.newline()
        print(', ')
      }
    }
...
print.make('switchBlock' ,function (ast ,opts) {
  print('{')
  print.incIndent()
  print.newline()
  print.statements(ast.body ,opts)
  print.decIndent()
  print.sep()
  print('}')
})
...
print.make('caseBodyBlock' ,function (arr) {
  print.caseBody(arr ,{oneLine: print.isOneLine.caseBody(arr)})
})
...
print.make('unaryPrefix' ,function (ast) {
  if (unaryNeedsSemi(ast.operator) && !print.printedSomething) print(';')
  print(ast.operator)
  if (unaryNeedsSpace(ast.operator)) print(' ')
  print.expression(ast.expression)
})
